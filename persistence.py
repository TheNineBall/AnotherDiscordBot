...
class User(Base):
    __tablename__ = 'User'
    user = sa.Column(sa.Integer, primary_key=True, unique=True)
...
class Persistence(object):
    ...
    async def addu(self, user: str, guild: str, exp: int = 0, level: int = -1):
        #async with self.database.transaction():
        u = await self.getu(user, guild)
        if len(u) > 0:
            q = self.users.update().values(rank=u['rank'] if level == -1 else level, exp=(int(u['exp']) + exp))
        else:
            q = self.users.insert().values(user=user, rank=0 if level == -1 else level, exp=exp, guild=guild)
